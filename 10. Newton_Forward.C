...
void forwardDifference(float x[], float y[], int n)
{
    for (int j = 1; j < n; j++)
    {
        for (int i = n - 1; i >= j; i--)
...
float newtonForwardInterpolation(float x[], float y[], int n, float value)
{
    float result = y[0];
    float term;
    for (int i = 1; i < n; i++)
...
// Interpolated value at x = 2.50 is: 12.500000